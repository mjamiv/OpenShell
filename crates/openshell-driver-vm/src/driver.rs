...
use openshell_vm::{RootfsVariant, extract_rootfs_variant_to};
...
fn validate_openshell_endpoint(endpoint: &str) -> Result<(), String> {
    ...
    let unreachable_from_vm = match host {
        Host::Domain(domain) => domain.eq_ignore_ascii_case("localhost"),
        Host::Ipv4(ip) => ip.is_loopback() || ip.is_unspecified(),
        Host::Ipv6(ip) => ip.is_loopback() || ip.is_unspecified(),
    };

    if unreachable_from_vm {
        return Err(format!(
            "openshell endpoint '{endpoint}' is not reachable from sandbox VMs; use a guest-reachable hostname such as http://host.containers.internal:<port> for local gvproxy development"
        ));
    }
...
mod tests {
    ...
    #[test]
    fn validate_openshell_endpoint_rejects_loopback_hosts() {
        let err = validate_openshell_endpoint("http://127.0.0.1:8080")
            .expect_err("loopback endpoint should fail");
        assert!(err.contains("not reachable from sandbox VMs"));

        let err = validate_openshell_endpoint("http://localhost:8080")
            .expect_err("localhost endpoint should fail");
        assert!(err.contains("host.containers.internal"));

        let err = validate_openshell_endpoint("http://[::1]:8080")
            .expect_err("ipv6 loopback endpoint should fail");
        assert!(err.contains("not reachable from sandbox VMs"));
    }
